...
DEFINE_string(instance, "/transmit_service/instance", "当前实例名称");

DEFINE_string(registry_host, "http://127.0.0.1:2379", "服务注册中心地址");
DEFINE_string(access_host, "127.0.0.1:9005", "当前实例的外部访问地址");
DEFINE_int32(listen_port, 9005, "Rpc服务器监听端口");
...
int main(int argc, char* argv[])
{
    ...
    IM::TransmitServiceBuilder tsbuilder;
    
    tsbuilder.init_mysql_client(FLAGS_mysql_user, FLAGS_mysql_pswd, FLAGS_mysql_host, 
                                FLAGS_mysql_db, FLAGS_mysql_cset, 
                                FLAGS_mysql_port, FLAGS_mysql_pool_count);

    tsbuilder.init_mq_client(FLAGS_mq_user, FLAGS_mq_password, FLAGS_mq_host, 
                                FLAGS_mq_msg_exchange, FLAGS_mq_msg_queue, FLAGS_mq_msg_bindkey);

    tsbuilder.init_discovery_client(FLAGS_registry_host, FLAGS_base_service, FLAGS_user_service);
    tsbuilder.init_rpc_server(FLAGS_listen_port, FLAGS_rpc_timeout, FLAGS_rpc_threads);
    tsbuilder.init_registry_client(FLAGS_etcd_host, FLAGS_base_service + FLAGS_instance, FLAGS_access_host);

    auto server = tsbuilder.build();
    server->start();
